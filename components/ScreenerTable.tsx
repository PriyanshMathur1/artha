...
const SIGNAL_STYLES: Record<Signal, string> = {
  ...
  'Accumulate': 'bg-cyan-50 text-cyan-700 border-cyan-200',
...
function SortIcon({ col, active, dir }: { col: string; active: string; dir: 'asc' | 'desc' }) {
  ...
  return dir === 'asc' ? <ArrowUp className="h-3 w-3 text-indigo-600" /> : <ArrowDown className="h-3 w-3 text-indigo-600" />;
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50">
            <th className="w-64 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
              Company
            </th>
            {cols.map((col) => (
              <th
                key={col.key}
                className="cursor-pointer select-none px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500 transition-colors hover:text-indigo-600"
...
function WatchlistStar({ ticker }: { ticker: string }) {
  ...
  return (
    <button
      ...
      className={`transition-colors ${added ? 'text-indigo-600' : 'text-slate-300 hover:text-indigo-600'}`}
    >
      <Star className={`h-3.5 w-3.5 ${added ? 'fill-indigo-600' : ''}`} />
    </button>
  );
}
    ...
    <tr className="group border-b border-slate-100 transition-colors hover:bg-slate-50">
        ...
        <div className="flex items-start gap-3">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-mono text-sm font-semibold text-slate-900">{row.ticker}</span>
              <span className={clsx('text-[10px] px-1.5 py-0.5 rounded border font-medium', SIGNAL_STYLES[row.signal])}>
                {row.signal}
              </span>
            </div>
            <div className="mt-0.5 max-w-[180px] truncate text-xs text-slate-600">{row.name}</div>
            <div className="mt-0.5 text-[10px] text-slate-400">{row.sector}</div>
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-3 w-36">
        <div className="space-y-1">
          ...
          <div className="relative h-1.5 overflow-hidden rounded-full bg-slate-200">
            ...
            />
            <div
              className="absolute -mt-0.5 h-2 w-2 rounded-full bg-white shadow"
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          <Link href={`/stock/${row.ticker}`} className="text-slate-400 hover:text-indigo-600">
