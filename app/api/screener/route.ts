...
export async function GET(req: NextRequest) {
  ...
  // Load DB-cached tokens (best-effort; app should still work without DB)
  let symbolToToken = new Map<string, string>();
  try {
    const dbTokens = await prisma.settings.findMany({
      where: { key: { startsWith: 'angel_token_' } },
    });
    symbolToToken = new Map<string, string>(
  ...
  const jwtToken = await getValidToken().catch(() => '');

  // Single Angel One bulk-quote call for all stocks
  const quotes = await bulkAngelQuote(tokenToSymbol, jwtToken);

  // Load cached fundamentals (best-effort)
  let fundamentalsMap = new Map<string, Awaited<ReturnType<typeof prisma.fundamentalsCache.findMany>>[number]>();
  try {
    const fundamentalsRecords = await prisma.fundamentalsCache.findMany({
      where: { symbol: { in: stocks.map((s) => s.ticker) } },
    });
    fundamentalsMap = new Map(fundamentalsRecords.map((f) => [f.symbol, f]));
  } catch {
